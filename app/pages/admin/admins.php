<?php
    ...
    //define('APP_FUNCTION', 'Admin - Manage admins');

	$username = $_SESSION['username'];
	$results = mysqli_query($db, "SELECT * FROM admins WHERE username = '$username'");
	$row = mysqli_fetch_assoc($results);

    $admin_res = mysqli_query($db, "SELECT * FROM admins ORDER BY Date DESC");
    ...
    <table class="white-space-nowrap w-full h-auto min-w-100 table-collapse">
        <thead class="w-full h-auto">
            <tr class="w-full h-auto justify-content-start">
                ...
                <th class="p-8 w-max justify-content-start align-items-center start-text color-black dark:color-white font-w-medium border-solid border-wb-1 border-w-0 border-slate-100 dark:border-slate-800">Username</th>
                ...
                <th class="p-8 w-max justify-content-start align-items-center start-text color-black dark:color-white font-w-medium border-solid border-wb-1 border-w-0 border-slate-100 dark:border-slate-800">Action</th>
            </tr>
        </thead>
        <tbody class="w-full h-auto">
            <?php foreach($admins as $key => $admin): ?>
            <tr class="w-full h-auto align-items-center justify-content-start">
                ...
                <td class="p-8 w-max justify-content-start align-items-center start-text border-solid border-wb-1 border-w-0 border-slate-100 dark:border-slate-800">
                    <div class="d-flex w-max align-items-center h-full gap-8">
                        <img src="<?php echo ROOT ?>/assets/images/avatars/male.jpg" class="border-round-full object-fit-cover object-position-center" style="height: 36px; width: 36px;">
                        <div class="d-flex flex-column line-h-normal align-items-start justify-content-center h-full">
                            <span class="text-capitalize"><?php echo $admin['username']; ?></span>
                        </div>
                    </div>
                </td>
                <td class="p-8 w-max justify-content-start align-items-center start-text border-solid border-wb-1 border-w-0 border-slate-100 dark:border-slate-800"><span><?php echo date('d M, Y', strtotime($admin['date'])); ?></span></td>

                <td class="p-8 w-max justify-content-start align-items-center start-text border-solid border-wb-1 border-w-0 border-slate-100 dark:border-slate-800">
                    <div style="display: flex; flex-direction: row; gap: .6rem;">
                        <a href="edit_user/<?php echo str_to_url($admin['username']); ?>" class="d-flex align-items-center justify-content-center p-8 bg-orange-500 hover:bg-orange-600 active:bg-orange-400 color-white border-round-md">
                        ...
                        <a class="d-flex align-items-center justify-content-center p-8 bg-red-500 hover:bg-red-700 active:bg-red-400 color-white border-round-md delete-user" data-username="<?php echo $admin['username']; ?>">
